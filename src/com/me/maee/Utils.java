...
public class Utils {
	...
	private static Vec rotatePoint2(float a,Vec position){
		...
		//b -  точка вокруг которой осуществл€етс€ вращение
		float x2,y2;
	...
	{	//рассто€ние от AB до C
		//возвращает также знак полуплоскости
		//+
		double dx = a.x-b.x;
		double dy = a.y-b.y;
		double D = dx * (c.y - a.y) - dy * (c.x-a.x); //ѕочему? необходимо про€снить этот момент, вывод формулы, например
